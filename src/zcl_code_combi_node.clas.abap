...
CLASS zcl_code_combi_node DEFINITION
  PUBLIC
  FINAL
  CREATE PRIVATE.
  ...
  PUBLIC SECTION.
    ...
    TYPES tt_children TYPE STANDARD TABLE OF REF TO zcl_code_combi_node WITH EMPTY KEY.
    ...
    CLASS-METHODS new_word    IMPORTING s TYPE string                  RETURNING VALUE(r) TYPE REF TO zcl_code_combi_node.
    CLASS-METHODS new_wseq    IMPORTING s TYPE string                  RETURNING VALUE(r) TYPE REF TO zcl_code_combi_node.
    CLASS-METHODS new_token   IMPORTING token_name TYPE string         RETURNING VALUE(r) TYPE REF TO zcl_code_combi_node.
    CLASS-METHODS new_regex   IMPORTING pattern TYPE string            RETURNING VALUE(r) TYPE REF TO zcl_code_combi_node.
    CLASS-METHODS new_seq     IMPORTING children TYPE tt_children      RETURNING VALUE(r) TYPE REF TO zcl_code_combi_node.
    CLASS-METHODS new_alt     IMPORTING children TYPE tt_children      RETURNING VALUE(r) TYPE REF TO zcl_code_combi_node.
    CLASS-METHODS new_opt     IMPORTING child TYPE REF TO zcl_code_combi_node RETURNING VALUE(r) TYPE REF TO zcl_code_combi_node.
    CLASS-METHODS new_star    IMPORTING child TYPE REF TO zcl_code_combi_node RETURNING VALUE(r) TYPE REF TO zcl_code_combi_node.
    CLASS-METHODS new_plus    IMPORTING child TYPE REF TO zcl_code_combi_node RETURNING VALUE(r) TYPE REF TO zcl_code_combi_node.
    CLASS-METHODS new_per     IMPORTING children TYPE tt_children      RETURNING VALUE(r) TYPE REF TO zcl_code_combi_node.
    CLASS-METHODS new_vers    IMPORTING child TYPE REF TO zcl_code_combi_node RETURNING VALUE(r) TYPE REF TO zcl_code_combi_node.
    CLASS-METHODS new_expr    IMPORTING name TYPE string               RETURNING VALUE(r) TYPE REF TO zcl_code_combi_node.
...
CLASS zcl_code_combi_node IMPLEMENTATION.
