...
"! ZCL_CODE_STMTS (statements) and ZCL_CODE_EXPRS (expressions),
...
"! that need them.
CLASS zcl_code_keywords DEFINITION
  PUBLIC
  FINAL
  CREATE PRIVATE.
  ...
  PUBLIC SECTION.
    ...
    "! Drop-in replacement for the static-list check in ZCL_CODE_METRICS.
  ...
  PRIVATE SECTION.
    ...
    CLASS-METHODS walk_node
      IMPORTING node TYPE REF TO zcl_code_combi_node.
...
CLASS zcl_code_keywords IMPLEMENTATION.
  ...
  METHOD build.
    ...
    collect_from_class( class_name    = 'ZCL_CODE_STMTS'
                        method_prefix = 'STMT_' ).
    collect_from_class( class_name    = 'ZCL_CODE_EXPRS'
                        method_prefix = 'EXPR_' ).
    mv_cached = abap_true.
  ENDMETHOD.
  ...
  METHOD collect_from_class.
    ...
    LOOP AT class_descr->methods INTO DATA(method_descr).
      ...
      DATA node TYPE REF TO zcl_code_combi_node.
      TRY.
  ...
  METHOD walk_node.
    ...
    " Mirrors zcl_code_combi_node->list_keywords( ) but ALSO follows
    " Expression references so we capture keywords contributed by
    " sub-expressions reached only via expr( ).
    CASE node->kind.
      WHEN zcl_code_combi_node=>c_kind_word
        OR zcl_code_combi_node=>c_kind_wseq.
        add_keyword( node->value ).

      WHEN zcl_code_combi_node=>c_kind_token
        OR zcl_code_combi_node=>c_kind_regex.
        " contributes nothing
        RETURN.

      WHEN zcl_code_combi_node=>c_kind_expr.
        ...
        DATA child TYPE REF TO zcl_code_combi_node.
        TRY.
            CALL METHOD ('ZCL_CODE_EXPRS')=>(method_name)
              RECEIVING
                r = child.
            walk_node( child ).
          CATCH cx_root.
