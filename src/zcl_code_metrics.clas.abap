class ZCL_CODE_METRICS definition
  public
  create public .
...
TYPES:
  ...
  class-methods CALCULATE
    importing
      !IS_PARSE_DATA type ZIF_CODE_PARSE_DATA=>TS_PARSE_DATA
...
CLASS ZCL_CODE_METRICS IMPLEMENTATION.
  ...
  METHOD classify_token.
    " Keyword classification now sourced from ZCL_CODE_KEYWORDS — derived 1:1
    " from the abaplint grammar (Combi.listKeywords over ZCL_CODE_STMTS/EXPRS).
    " The static fill_statements list has been removed.
    IF zcl_code_keywords=>is_keyword( i_token ) = abap_true.
      rv_kind = 'OPERATOR'.
    ELSE.
      rv_kind = 'OPERAND'.
    ENDIF.
