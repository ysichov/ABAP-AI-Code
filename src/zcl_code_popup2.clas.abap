...
CLASS ZCL_CODE_POPUP2 IMPLEMENTATION.
  ...
  endmethod.
      ...
      " the final LLM call вЂ” returns the fully assembled prompt in rs_result-final_prompt.
      ...
      " Do NOT append messages here вЂ” will append after streaming completes (with FINAL message).

      " If runner returned without a final_prompt (direct lookup / pure code search),
      " display the result immediately вЂ” no need to stream via Python.
      IF ls_stream_prep-final_prompt IS INITIAL.
        ...
        ELSE.
          " Non-code answer (search result list etc.) вЂ” convert to HTML with clickable links
      ...
      " Launch Python via cmd.exe (trusted by SAP GUI Security).
      " "start /b" runs Python without opening a new console window.
      DATA(lv_script) = mv_agents_path && '\llm_stream.py'.
      DATA(lv_params) = |/c start "" /b python "{ lv_script }" "{ mv_stream_prompt_file }" "{ mv_stream_response_file }"|.
      ...
      WHILE abap_true = abap_true.
        ...
        CLEAR lt_resp_lines.
        " Check file size first вЂ” skip upload if file still empty (avoids "0 of 0" dialog)
        ...
        " Do NOT condense вЂ” preserves code indentation and formatting

        " While file is still empty вЂ” show "Waiting LLM answer..." with elapsed seconds
        ...
        IF lv_resp_text IS INITIAL AND lv_stream_done = abap_false AND lv_stream_error = abap_false.
          ...
          lv_resp_html = |вЏі Waiting LLM answer... { lv_wait_int }s|.
        ELSE.
          " Escape HTML special chars вЂ” Python wrote plain text, ABAP escapes for HTML display.
        ...
        IF lv_stream_done = abap_true AND lv_stream_error = abap_false.
          " Final answer received вЂ” render markdown (headings, lists, code blocks)
      ...
      IF lv_stream_done = abap_true.
        " lv_resp_text is plain text (UTF-8 read by gui_upload в†’ Unicode).
        " Register as-is вЂ” no HTML encoding in the history log.
      ...
      ENDIF.
      " Do NOT call display_status вЂ” it would overwrite the streamed answer.
      IF lv_stream_error = abap_true.
        MESSAGE 'Stream error вЂ” check C:\soft\stream\llm_stream.log' TYPE 'S'.
    ...
    " Source-only result (direct lookup / SHOW command) в†’ ABAP editor.
    ...
    " Non-code search result (error / "similar classes") вЂ” convert to HTML with clickable links.
    ...
    " For plain LLM text answers do not append source вЂ” it was not requested.
    display_answer(
