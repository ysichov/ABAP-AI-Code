...
"! Returns ZCL_CODE_COMBI_NODE trees that can be walked via list_keywords( ).
CLASS zcl_code_combi DEFINITION
  PUBLIC
  FINAL
  CREATE PRIVATE.
  ...
  PUBLIC SECTION.
    ...
    TYPES tt_nodes TYPE zcl_code_combi_node=>tt_children.
    ...
    CLASS-METHODS str
      ...
      RETURNING VALUE(result) TYPE REF TO zcl_code_combi_node.
    ...
    CLASS-METHODS opt
      IMPORTING child         TYPE REF TO zcl_code_combi_node
      RETURNING VALUE(result) TYPE REF TO zcl_code_combi_node.
    ...
    "! a string name and resolve via dynamic call zcl_code_exprs=>expr_<name>( ).
    CLASS-METHODS expr
...
CLASS zcl_code_combi IMPLEMENTATION.

  METHOD str.
    " Mirrors combi.ts: if (s.indexOf(" ") > 0 || s.indexOf("-") > 0) WordSequence else Word
    IF s CS ` ` OR s CS `-`.
      result = zcl_code_combi_node=>new_wseq( s ).
    ELSE.
      result = zcl_code_combi_node=>new_word( s ).
    ENDIF.
  ENDMETHOD.

  METHOD tok.
    result = zcl_code_combi_node=>new_token( token_name ).
  ENDMETHOD.

  METHOD regex.
    result = zcl_code_combi_node=>new_regex( pattern ).
  ENDMETHOD.

  METHOD seq.
    result = zcl_code_combi_node=>new_seq( children ).
  ENDMETHOD.

  METHOD alt.
    result = zcl_code_combi_node=>new_alt( children ).
  ENDMETHOD.

  METHOD opt.
    result = zcl_code_combi_node=>new_opt( child ).
  ENDMETHOD.

  METHOD star.
    result = zcl_code_combi_node=>new_star( child ).
  ENDMETHOD.

  METHOD plus.
    result = zcl_code_combi_node=>new_plus( child ).
  ENDMETHOD.

  METHOD per.
    result = zcl_code_combi_node=>new_per( children ).
  ENDMETHOD.

  METHOD ver.
    result = zcl_code_combi_node=>new_vers( child ).
  ENDMETHOD.

  METHOD expr.
    result = zcl_code_combi_node=>new_expr( name ).
  ENDMETHOD.
