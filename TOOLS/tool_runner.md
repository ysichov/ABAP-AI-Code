...
- Call read_sap_object before modify_sap_object. Do NOT read_sap_object just to feed review_sap_code the SAME object's source - review_sap_code reads it itself, so pre-reading only floods the context. (Reading a program once to discover which Z objects it USES, before reviewing those, is a different, allowed case - see below.)
